...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - globallydistributeddatabase Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-globallydistributeddatabase"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-globallydistributeddatabase",
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "globallydistributeddatabase")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated globallydistributeddatabase controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "globallydistributeddatabase")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup globallydistributeddatabase controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_globallydistributeddatabase(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_globallydistributeddatabase(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_globallydistributeddatabase(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_globallydistributeddatabase(mgr, o)
}
